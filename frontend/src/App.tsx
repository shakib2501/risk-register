...
import { createMitigation, createRisk, deleteMitigation, deleteRisk, getMitigations, getRisks, updateMitigation, updateRisk, type Mitigation, type Risk, type RiskCategory, type RiskInput, type RiskStatus } from './api'
import './App.css'
...
function App() {
  ...
  return <main className="app-shell">
    <header className="page-header"><div><p className="eyebrow">Governance workspace</p><h1>Risk Register</h1><p className="subtitle">Track, assess, and mitigate the risks that matter.</p></div><button type="button" className="primary-button" onClick={() => setFormRisk(null)}>Add risk</button></header>
    ...
    {formRisk !== undefined && <div className="modal-backdrop" role="presentation"><section className="risk-form" role="dialog" aria-modal="true" aria-labelledby="risk-form-heading"><div className="form-header"><h2 id="risk-form-heading">{formRisk ? 'Edit risk' : 'Add a risk'}</h2><button type="button" className="close-button" aria-label="Close form" onClick={() => setFormRisk(undefined)}>×</button></div><form key={formRisk?.id ?? 'new'} onSubmit={saveRisk}><label>Title<input name="title" defaultValue={formRisk?.title} required /></label><label>Description<textarea name="description" defaultValue={formRisk?.description} required /></label><label>Category<select name="category" defaultValue={formRisk?.category ?? 'OPERATIONAL'}>{categories.map((item) => <option key={item} value={item}>{format(item)}</option>)}</select></label><label>Owner<input name="owner" defaultValue={formRisk?.owner} required /></label><div className="form-grid"><label>Likelihood<select name="likelihood" defaultValue={formRisk?.likelihood ?? 3}>{[1, 2, 3, 4, 5].map((value) => <option key={value}>{value}</option>)}</select></label><label>Impact<select name="impact" defaultValue={formRisk?.impact ?? 3}>{[1, 2, 3, 4, 5].map((value) => <option key={value}>{value}</option>)}</select></label></div><label>Status<select name="status" defaultValue={formRisk?.status ?? 'OPEN'}>{statuses.map((item) => <option key={item} value={item}>{format(item)}</option>)}</select></label><div className="form-actions"><button type="button" className="secondary-button" onClick={() => setFormRisk(undefined)}>Cancel</button><button type="submit" className="primary-button">Save risk</button></div></form></section></div>}
  </main>
}
