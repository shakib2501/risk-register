...
type RiskAction = 'ADD_MITIGATION' | 'EDIT' | 'DELETE'
...
function App() {
  ...
  function performRiskAction(risk: Risk, action: RiskAction) {
    if (action === 'ADD_MITIGATION') openMitigationDetails(risk)
    if (action === 'EDIT') openEditRisk(risk)
    if (action === 'DELETE') void removeRisk(risk)
  }
  ...
  async function saveRisk(event: FormEvent<HTMLFormElement>) {
    ...
    const input: CreateRiskInput = {
      ...
      nextReviewDate: String(values.get('nextReviewDate')) || undefined,
